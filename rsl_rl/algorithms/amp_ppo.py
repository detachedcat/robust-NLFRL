...
class AMPPPO:
    ...
    def _split_storage_sample(self, sample):
        """Return (core_sample, extra_dict). Subclasses may unpack extended storage fields."""
        return sample, {}
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            sample, extra = self._split_storage_sample(sample)
            (
                obs_batch,
                critic_obs_batch,
                actions_batch,
            ...
            if self.symmetry and self.symmetry["use_data_augmentation"]:
                ...
                extra = self._repeat_extra_for_augmentation(extra, num_aug)
            ...
            loss, batch_metrics = self._augment_loss_before_backward(loss, batch_ctx)
            if not torch.isfinite(loss):
                skipped_non_finite_batches += 1
                continue
            ...
            self._zero_extra_optimizer_grads()
            loss.backward()
            # -- For RND
            if self.rnd:
            ...
            self.optimizer.step()
            self._extra_optimizer_steps(batch_ctx)
            ...
            self._accumulate_batch_metrics(batch_metrics)

        # -- For PPO
        num_updates = max(effective_updates, 1)
        self._last_effective_updates = num_updates
        ...
        return self._merge_update_metrics(loss_dict)
