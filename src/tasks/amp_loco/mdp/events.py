...
class MotionResetManager:
    ...
    def __init__(self) -> None:
        ...
        self.walk_clip_starts: dict[str, torch.Tensor] = {}
        self.walk_clip_lengths: dict[str, torch.Tensor] = {}
        self.recovery_clip_starts: dict[str, torch.Tensor] = {}
        self.recovery_clip_lengths: dict[str, torch.Tensor] = {}
        self.reset_frame_idx: torch.Tensor | None = None
        self.reset_use_recovery: torch.Tensor | None = None
    ...
    def init(
        ...
        frames, clip_starts, clip_lengths = self._concat_frames(loader.motion_data, env.device)
        self.walk_run_frames[motion_dir] = frames
        self.walk_clip_starts[motion_dir] = clip_starts
        self.walk_clip_lengths[motion_dir] = clip_lengths
        motion_count = frames["root_pos"].shape[0]
        ...
        if loader.motion_data_recovery:
            rec_frames, rec_starts, rec_lengths = self._concat_frames(
                loader.motion_data_recovery, env.device
            )
            self.recovery_frames[motion_dir] = rec_frames
            self.recovery_clip_starts[motion_dir] = rec_starts
            self.recovery_clip_lengths[motion_dir] = rec_lengths
            recovery_count = rec_frames["root_pos"].shape[0]
            print(
                f"[MotionResetManager] Loaded {len(loader.motion_data_recovery)} recovery clips, "
                f"{recovery_count} frames from {recovery_dir}"
            )

        if self.reset_frame_idx is None or self.reset_frame_idx.shape[0] != env.num_envs:
            self.reset_frame_idx = torch.zeros(env.num_envs, dtype=torch.long, device=env.device)
            self.reset_use_recovery = torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
    ...
    def reset(
        ...
        if len(normal_ids) > 0:
            self._write_reset_state(
                env, normal_ids, self.walk_run_frames[motion_dir], asset_cfg, use_recovery=False
            )

        # Reset delay envs with recovery data (fallback to walk/run if unavailable).
        if len(delay_ids) > 0:
            recovery = self.recovery_frames.get(motion_dir)
            use_recovery = recovery is not None
            frames = recovery if use_recovery else self.walk_run_frames[motion_dir]
            self._write_reset_state(env, delay_ids, frames, asset_cfg, use_recovery=use_recovery)
    ...
    def _write_reset_state(
        ...
        if self.reset_frame_idx is None or self.reset_frame_idx.shape[0] != env.num_envs:
            self.reset_frame_idx = torch.zeros(env.num_envs, dtype=torch.long, device=env.device)
            self.reset_use_recovery = torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
        self.reset_frame_idx[env_ids] = idx
        self.reset_use_recovery[env_ids] = use_recovery
    ...
    @staticmethod
    def _concat_frames(
        motions: list[dict], device: torch.device | str
        ...
        starts = torch.tensor(clip_starts, dtype=torch.long, device=device)
        lengths = torch.tensor(clip_lengths, dtype=torch.long, device=device)
        return frames, starts, lengths
